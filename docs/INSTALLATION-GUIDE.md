...
This document guides you through setting up the application, making it secure with HTTPS, having backups and how to upgrade it.
...
1. Create a folder
    ```shell
    mkdir /opt/simple-music-manager
    cd /opt/simple-music-manager
    ```
2. Download the `docker-compose.yaml` and the nginx `default.conf` file:
    ```shell
    curl -L https://raw.githubusercontent.com/lchristmann/simple-music-manager/main/docker-compose.yaml -o docker-compose.yaml
   curl -L https://raw.githubusercontent.com/lchristmann/simple-music-manager/main/docker/deployment/nginx/default.conf -o docker/deployment/nginx/default.conf
    ```
   > You can customize the server port that the simple-music-manager will be running on by changing one line in the `docker-compose.yaml`.<br>
...
3. Download the `.env.example` and save it as `env` file
    ```shell
    curl -L https://raw.githubusercontent.com/lchristmann/simple-music-manager/main/.env.example -o .env
    ```
...
5. Create the Docker network and start the services
    ```shell
    docker network create simple-music-manager-network
    docker compose up -d
    ```
...
Now you can create your first admin user to log in:
...
1. Point your (sub)domain to your server's public IP, e.g.
   ```text
   A music.example.com 192.168.1.1
   ```
...
1. Edit the `.env` file and set the `APP_URL` to your (sub)domain prefixed with `https://` and add a variable `ASSET_URL` with the same value.
   ...
   nano .env
   ```
2. Edit the `docker-compose.yaml` file and comment out the entire ports section. (This ensures the `web` service is only reachable internally in the `simple-music-manager-network` Docker network)
   ```shell
...
4. Set up the [Nginx Proxy Manager](https://nginxproxymanager.com/)
    ...
    2. Create a `docker-compose.yaml` file like this (from the [official Quick Setup guide](https://nginxproxymanager.com/guide/#quick-setup))
       ...
       ```
       Also paste this, to put the Nginx Proxy Manager into the `simple-music-manager-network` Docker network.
       The Proxy Manager is the single point of access to the application now.
       ```yaml
           # This one level nested under the 'app' service
           networks:
             - simple-music-manager-network
    ...
    6. Find the container name of your app's `web` service, usually `simple-music-manager-web-1`.
       ```shell
       cd /opt/simple-music-manager
    ...
    7. In the Nginx Proxy Manager visit Dashboard > Proxy Hosts > Add Proxy Hosts:
        - "Details" Tab
          ...
          | Setting               | Value                      | Example                      | Explanation                                                                        |
                   |-----------------------|----------------------------|------------------------------|------------------------------------------------------------------------------------|
          | Domain Names          | your domain name           | `music.example.com`          |                                                                                    |
          | Scheme                | http (yes!!)               |                              | This is only Docker network internal, we'll force HTTPS for internet traffic later |
          | Forward Hostname / IP | the `web` container's name | `simple-music-manager-web-1` | See previous step                                                                  |
          | Forward Port          | 80                         |                              |                                                                                    |
...
```shell
/opt/simple-music-manager/backup.sh
```

You can now copy the resulting folder `/opt/simple-music-manager-backups/<today's date and time>` somewhere safe (another server, your local machine, cloud storage,...).<br>
It contains two backup dump files `postgres.dump` and `storage-volume.tar.gz`

### Set Up Periodic Backups

Once your backup script is in place, you should **run it automatically at regular intervals** to ensure you always have recent copies of your data.
...
2. Add the following line to run the backup script every Sunday at 03:30:
    ```text
    30 3 * * 0 /opt/simple-music-manager/backup.sh >> /var/log/simple-music-manager-backup.log 2>&1
    ```
    ...
    Each run creates a new folder `/opt/simple-music-manager-backups/<today's date and time>`.<br>
    Output and errors are logged to `/var/log/simple-music-manager-backup.log`.

### Restore from a Backup

If something goes wrong, and you need to recover your data, you can restore both the database and the storage volume from a previously created backup folder.

1. Enter the simple-music-manager's directory.
    ```shell
    cd /opt/simple-music-manager
    ```
2. Choose a backup to restore from.<br>List available backups:
    ```shell
    ls /opt/simple-music-manager-backups
    ```
   Pick the folder you need, for example run (**insert your backup's datetime here!**):
    ```shell
    RESTORE_DIR="/opt/simple-music-manager-backups/2025-12-29_22-19"
    ```
...
5. Restore the Laravel storage volume
    ```shell
   docker run --rm \
      -v simple-music-manager_laravel-storage-production:/data \
...
Your Simple Music Manager application should now be restored to the exact state at the moment of the backup.

## Upgrade Guide

SSH into your server.

1. Update the `leanderchristmann/simple-music-manager` docker image version in the `docker.yaml` file to the current (or some other desired) release:
   ```shell
   cd /opt/simple-music-manager
   nano docker-compose.yaml
   ```
